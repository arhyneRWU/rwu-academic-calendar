...
(() => {
  ...
  function ics(termId, rows) {
    ...
    for (const c of rows) {
      ...
      // One EXDATE property per date rather than one comma-joined list.
      // Both are legal; Outlook is reliable with the first form and not with
      // the second, and a long list also has to be folded, which is another
      // thing to get wrong.
      for (const d of s.exdate) out.push(`EXDATE:${at(d)}`);
      ...
      // Dates the rule cannot express: the day swap's gained Tuesday, and
      // every date in "only on dates I list" mode. These used to be RDATE,
      // which is where the whole series fell apart in Outlook -- Outlook's
      // recurrence model has no way to represent "and also this one date",
      // so rather than keep the pattern and drop the extra, it abandons the
      // pattern and imports every meeting as a separate appointment. Each
      // extra is now its own event, which is what it actually is.
      const extra = s.byRule.length ? s.rdate.filter(d => d !== first) : s.rdate;
      for (const d of extra) {
        out.push('BEGIN:VEVENT',
          `UID:${key}${n > 1 ? '-' + n : ''}-${stamp(d)}@rwu-academic-calendar`,
          `DTSTAMP:${new Date().toISOString().replace(/[-:]/g,'').replace(/\.\d+/,'')}`,
          'SEQUENCE:0',
          `DTSTART:${at(d)}`,
          `DTEND:${stamp(d)}T${c.end.replace(':','')}00`,
          `SUMMARY:${esc(c.name)}`);
        if (c.room) out.push(`LOCATION:${esc(c.room)}`);
        out.push('DESCRIPTION:' + esc(
          (s.byRule.length ? 'Extra meeting: this date runs a different '
                             + 'weekday\'s timetable. ' : '')
          + 'Generated from the unofficial RWU academic calendar. Verify '
